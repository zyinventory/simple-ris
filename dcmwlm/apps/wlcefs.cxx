    ...
    fileOutputStream( NULL ), app( NULL ), cmd( NULL ), dataSource( dataSourcev ), argc(paramArgc), argv(paramArgv)
{
  ...
  cmd->addGroup("returned character set options:", LONGCOL, SHORTCOL+2);
    ...
    cmd->addOption("--return-default-gb18030",	  "-cs2",       "if no specific character set, then return GB18030");  // zy modified, add
  ...
  if( app->parseCommandLine( *cmd, argc, argv, OFCommandLine::ExpandWildcards ) )
  {
    ...
    if( cmd->findOption("--return-default-gb18030") )  opt_returnedCharacterSet = RETURN_CHARACTER_SET_DEFAULT_GB18030;	//zy modified, add
    cmd->endOptionBlock();
    cmd->beginOptionBlock();
