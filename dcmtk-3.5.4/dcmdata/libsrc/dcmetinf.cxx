        ...
        else if (errorFlag.good() && fTransferState != ERW_ready)
        {
            ...
            if (fTransferState == ERW_inWork && Length == 0)
            {
                ...
                else
                {
                    ...
                    if (errorFlag.good())
                    {
#ifdef DO_NOT_IGNORE_META_GROUP_LENGTH
                        /* this is the default behavior of DCMTK 3.5.4 */
                ...
                ((errorFlag == EC_CorruptedData) && (Length == DCM_UndefinedLength))))
            {
                        ...
                        !lastElementComplete))
                {
                    ...
                    if (lastElementComplete)
                    {
                        ...
                        {
                            ofConsole.lockCerr() << "DcmMetaInfo: Invalid Element " << newTag << " found in Meta Information Header" << endl;
                            ofConsole.unlockCerr();
                        }
                    } else {
