...
COMMONLIB_API bool SelectValidPublisher(const char *ini_path, string &valid_publisher, bool opt_verbose)
{
    map<string, int> pubs;
        ...
        if(psec)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    valid_publisher = "error:任务详细状态错误";
...
COMMONLIB_API bool TryPublishJDF(bool opt_verbose)
{
    ...
    publisher.reserve(255);
    if(SelectValidPublisher(filter.c_str(), publisher, opt_verbose))
    {
        ...
        {
            displayErrorToCerr("TryPublishJDF() append publisher to jdf error", &errstrm);
        }
    }
    return false;
}
